...
e.g. "logical", "integer", "double", "complex", "character", "raw" and "list", "NULL", "closure" (function), "special" and "builtin" (basic functions and operators), "environment", "S4" (some S4 objects) and others
"class()"
...
# double is any numbers (incl those with decimal places), whereas integer is specifically for whole numbers. When we create the string using a vector using c() or sequence, then it saves the values as a "double", however the colon operator saves the values as integers
...
"Paste" sticks the items contained in the "" together (like pasting things together), on one line.
"Cat" can also stick the items contained in the "" together, however it can also create new lines using "\n"
Finally, "Glue" allows expressions which are enclosed by the brackets to be evaluated as R code
...
'1 <- 1' causes an error as it essentially tries to assign the value '1' a value of '1' which doesn't work as you can only assign values to variables, not to literal values like numbers.

= is the assignment operator. It assigns a value to a variable
This means "assign the value 5 to the variable x."
...
Character Vectors store text as strings, where each element is a string - useful for RAW TEXT DATA
Conversely, factors store categorical data, and are typically stored as integers with associated levels (unique categories) - usefule for STATISTICAL MODELLING, PLOTTING (where categories matter)
...
Matrices and arrays are both ways to store multi-dimensional data, however a matrix is a 2-D data strucutre which has rows and columns, and all elements must be of the same type, whilst an array can have one or more dimension (1-D, 2-D, 3-D,...) - it is a generalisation of matrices to higher dimensions.
...
When you don't provide enough values for an array, R recycles (repeats) the given values to fill the array. If it doesn't divide nicely, then R will recycle the elements with a warning.

3D arrays are displayed by 2D arrays, where ", , z" displays the layer of the 3D array being shown. 
...
Vectors are an atomic data structure in R that contain elements of the same type (e.g. all numeric, all character, all logical etc). Conversely, lists are a recursive data structure that can contain elements of different types. Each element of a list can be a vector, a number, a string, another list, a matrix or a function. It is useful for grouping hetrogeneous data.
...
A *Data Frame* is the standard R data structure (part of base R) for tabular data that behaves like a list of equal-length vectors. A Data Frame displays all the data (which can get overwhelming). A *tibble* is part of tidyverse. It is a list of vectors that has better defaults for usability, and only displays the first 10 rows and m columns (as many columns as fit on the screen). Tibbles do NOT convert strings to factors by default

*tibble()* creates a table by specifying columns as vectors.

*tribble()* stands for transposed tibble, and is designed for creating small tibbles in a readable row-wise format (input looks more like a spreadsheet, with data entries happening row by row)
...
str() = STRUCTURE
A base R function which shows the type, size and first few values of each element/component

glimpse() 
Part of the tibble package which is designed specifically for data frames and tibbles. It displays the data horizontally with one row per column (shows variable names, data types and preview of data)

read_csv() is designed specifically to read CSV files, and assumes that the deliminater is a comma

read_delim is a more general function which can read delimited files, which allows the delimiter to be specified through the delim argument
...
Created the "my_weather_data.csv" file
It is good practice to read the data back after saving to ensure that it saved correctly and to minimise errors.
...
"Sea" is a character string, therefore requires the brackets
"==" is required to search for the requirement, as "=" assigns values to variables.
...
No the results are not the same as the proportion depends on how many babies were born in a given year, hence if a lot more babies were born in a given year, but only a few more babies of a given name, then proportionately, there would have been less babies born with that name.
...
table1 is tidy as it contains all the data but is still readable
table2 is less readable and it is harder to distinguish the cases and population
table3 could be suitable, however it is marginally harder to read the "rate" compared the the number of cases and population number as the data isn't formatted to be separated at a given character (and it is therefore a bit messier)
table4 removes the number of cases so there is some data missing
table5 also removes the number of cases so there is some data missing
...
cholesterol_data_long <- cholesterol_data |>
  pivot_longer(
    cols = c(`oat_bran`, `oat_bran_free`),    # Columns to pivot
    names_to = "Diet_type",           # Name for the key column
    values_to = c("Cholesterol_level")          # Name for the value column
  )
cholesterol_data_long

# Task 2 - Calculate summary statistics:
## count the rows where oat bran and oat bran free??
cholesterol_data_long |>
  filter(Diet_type == "oat_bran") |>
  nrow()

cholesterol_data_long |>
  filter(Diet_type == "oat_bran_free") |>
  nrow()

cholesterol_data_long |>
  filter(Diet_type == "oat_bran") |>
  pull(Cholesterol_level) |>
  mean()
...
The Oat Bran diet appears to result in lower cholesterol with a mean of 4.508 compared to the mean cholesterol for those who had the Oat Bran Free diet.

The standard deviations show that there was less variability in the cholesterol in those who had the Oat Bran Free diet, compared to the variability in cholesterol in those who were on the Oat Bran diet.
...
table2 & table 3 was tricky - where are the tables??
